...
class Lorenz63:
    ...
    def flow(self, x: np.ndarray) -> np.ndarray:
        ...
        """
        return np.array([self.sigma * (x[1] - x[0]), x[0] * (self.rho - x[2]) - x[1],
...
class ChuaCircuit:
    ...
    def __init__(
            self, alpha: float = 9, beta: float = 100 / 7, a: float = 8 / 7, b: float = 5 / 7,
            dt: float = 0.05
    ) -> None:
        """Define the system parameters
    ...
    def flow(self, x: np.ndarray) -> np.ndarray:
        ...
        return np.array(
            [
                self.alpha
                * (x[1] - x[0] + self.b * x[0] + 0.5 * (self.a - self.b) * (
...
class Thomas:
    ...
    def flow(self, x: np.ndarray) -> np.ndarray:
        ...
        """
        return np.array([-self.b * x[0] + np.sin(x[1]), -self.b * x[1] + np.sin(x[2]),
...
class Rucklidge:
    ...
    def flow(self, x: np.ndarray) -> np.ndarray:
        ...
        """
        return np.array(
            [-self.kappa * x[0] + self.lam * x[1] - x[1] * x[2], x[0], -x[2] + x[1] ** 2])
...
class SimplestQuadraticChaotic:
    ...
    def simulate(self, time_steps: int,
                 starting_point: np.ndarray = np.array([-0.9, 0.0, 0.5])) -> np.ndarray:
...
class SimplestCubicChaotic:
    ...
    def simulate(self, time_steps: int,
                 starting_point: np.ndarray = np.array([0.0, 0.96, 0.0])) -> np.ndarray:
...
class SimplestPiecewiseLinearChaotic:
    ...
    def simulate(self, time_steps: int,
                 starting_point: np.ndarray = np.array([0.0, -0.7, 0.0])) -> np.ndarray:
...
class DoubleScroll:
    ...
    def simulate(self, time_steps: int,
                 starting_point: np.ndarray = np.array([0.01, 0.01, 0.0])) -> np.ndarray:
...
class UedaOscillator:
    ...
    def __init__(self, b: float = 0.05, A: float = 7.5, omega: float = 1,
                 dt: float = 0.05) -> None:
        """Define the system parameters.
    ...
    def flow(self, x: np.ndarray) -> np.ndarray:
        ...
        """
        return np.array(
            [x[1], -(x[0] ** 3) - self.b * x[1] + self.A * np.sin(self.omega * x[2]), 1])
...
class KuramotoSivashinsky:
    ...
    def _prepare(self) -> None:
        ...
        k = (
                np.transpose(
                    np.conj(
                        np.concatenate(
                            (np.arange(0, self.dimensions / 2), np.array([0]),
    ...
    def simulate(self, time_steps: int, starting_point: np.ndarray | None = None) -> np.ndarray:
        ...
        if starting_point is None:
            ...
            x = self.system_size * np.transpose(
                np.conj(np.arange(1, self.dimensions + 1))) / self.dimensions
...
class Lorenz96:
    ...
    def __init__(self, dimensions: int = 30, force: float = 8, dt: float = 0.05) -> None:
        """Define the system parameters.
    ...
    def simulate(self, time_steps: int, starting_point: np.ndarray | None = None) -> np.ndarray:
        ...
        if starting_point is None:
            ...
            if starting_point.shape[0] != self.dimensions:
                raise ValueError(
                    f"starting_point wrong dimension: Expected {self.dimensions} but got "
                    f"{starting_point.shape[0]}"
                )
